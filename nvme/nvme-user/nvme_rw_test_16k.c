...
int main(int argc, char **argv)
{
	...
	void *data_wr;
	void *data_rd;
	if ( posix_memalign(&data_wr, 16384, PAGE_PER_CMD*16384) ) {
		fprintf(stderr, "cannot allocate io payload for data_wr\n");
		return 0;
	}
	if ( posix_memalign(&data_rd, 16384, PAGE_PER_CMD*16384) ) {
	...
	unsigned int i = 0;
	unsigned int j = 0;
	for (i = 0; i < 16384*PAGE_PER_CMD; i++) {
	...
	for (i = 0; i < ITER; i++) {
		io.slba = LBA_BASE + (i*PAGE_PER_CMD*4);
		io.addr = (unsigned long)data_wr;
	...
	fprintf(stdout, "\n");
	fprintf(stdout, "TIME_WR: %lf, bw=%lf B/s\n", time_elapsed, (double)16384*PAGE_PER_CMD*ITER/time_elapsed);
#endif	// EN_WR
	...
	#if (EN_CHECK_ERR)
		for (j = 0; j < 16384*PAGE_PER_CMD; j++) {
			n_byte++;
